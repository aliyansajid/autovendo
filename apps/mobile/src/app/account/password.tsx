...
export default function ChangePasswordScreen() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError("Das Passwort konnte nicht geändert werden. Prüfen Sie Ihr aktuelles Passwort.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <View style={[styles.root, { backgroundColor: C.background }]}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
          ...
          <TextField label="Aktuelles Passwort" value={current} onChangeText={setCurrent} secure icon="lock" />
          <TextField label="Neues Passwort" value={next} onChangeText={setNext} secure icon="lock.rotation" placeholder="Mindestens 8 Zeichen" />
          <TextField label="Neues Passwort bestätigen" value={confirm} onChangeText={setConfirm} secure icon="lock.rotation" onSubmitEditing={submit} returnKeyType="done" />
          <Button label="Passwort speichern" onPress={submit} loading={saving} size="lg" fullWidth style={{ marginTop: Spacing[3] }} />
        </ScrollView>
