    ...
    <div className="flex-1 space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <Receipt className="size-6" /> Subscriptions
        </h2>
      </div>
                  ...
                  <TableRow key={sub.id}>
                    <TableCell className="font-medium">
                      <div className="flex flex-col">
                        <span>{dealer?.companyName || user?.name || "Unknown"}</span>
                    ...
                    <TableCell>
                      {sub.trialStart || sub.trialEnd ? (
                        <div className="flex flex-col text-xs">
                          <span>Start: {sub.trialStart ? new Date(sub.trialStart).toLocaleDateString("de-CH") : "-"}</span>
                          <span>End: {sub.trialEnd ? new Date(sub.trialEnd).toLocaleDateString("de-CH") : "-"}</span>
                        </div>
                      ) : (
                    ...
                    <TableCell>
                      {sub.cancelAtPeriodEnd ? (
                        ...
                        <Badge variant="outline" className="text-green-600 border-green-200 bg-green-50">On</Badge>
                      )}
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-col text-xs">
                        {sub.canceledAt && (
                          <span className="text-destructive">Canceled: {new Date(sub.canceledAt).toLocaleDateString("de-CH")}</span>
                        )}
                        {sub.endedAt && (
                          <span className="text-muted-foreground">Ended: {new Date(sub.endedAt).toLocaleDateString("de-CH")}</span>
                        )}
