...
import type { ListingProps } from "@/components/listing-card";

function vehicleToListingProps(item: VehicleListItem): ListingProps {
  return {
    id: item.id,
    image: getImageUrl(item.images[0]),
    badge: item.vehicleCondition ? formatEnumLabel(item.vehicleCondition) : undefined,
    ...
    garageLocation: [item.dealer.zipCode, item.dealer.city].filter(Boolean).join(" "),
  };
}
...
export default async function HomePage() {
  ...
  const listings: ListingProps[] = vehiclesResult.vehicles.map(vehicleToListingProps);
  const garages = dealersResult.dealers.map((d) => ({
    id: d.id,
    name: d.companyName,
    image: d.logo ? (d.logo.startsWith("http") ? d.logo : getImageUrl(d.logo)) : "/placeholder-car.jpg",
